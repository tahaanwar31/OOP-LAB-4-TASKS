...
class Novel {
    ...
    public:
        ...
        Novel() : bookName("Unknown"), cost(0.0), novelID(++IDcounter)  {
            availableStock = new int(30);
        }
        Novel(string bookName, float cost, int availableStock) : bookName(bookName), cost(cost), novelID(++IDcounter) {
            this->bookName = bookName;
            this->cost = cost;
            this->availableStock = new int(availableStock);
        }
        Novel(const Novel& source) : novelID(++IDcounter) {
        ...
        void displayNovel() {
            cout << "ID: " << novelID << endl << "Title: " << bookName << endl << "Price: $" << cost << endl << "Stock: " << *availableStock << endl;
        }
        ~Novel() {
